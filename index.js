...
function game(playerMove) {
  const gamePlay = Math.floor(Math.random() * 3) + 1;
  ...
  if (gamePlay == 1) {
    ...
    gameDecision = "scissors";
  }

  if (playerMove === "rock") {
    if (gameDecision === "rock") {
      return "Draw!" + " against " + gameDecision;
    } else if (gameDecision === "paper") {
      return "You lost!" + " against " + gameDecision;
    } else if (gameDecision === "scissors") {
      return "You won!" + " against " + gameDecision;
    }
...
function annoyingSong(number) {
  ...
  // start at number
  // until number = 0
  // and we want to countdown so integer minus minus

  let i;
  for (i = number; i >= 0; i--) {
    // if we return here it will just return the assigned integer number.
    // we want to console log each iteration of the loop
    console.log(i);
  }
}
